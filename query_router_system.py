...
class QueryFeatureExtractor:
    ...
    def extract_features(self, query: str) -> QueryFeatures:
        ...
        # Pattern matching
        has_multi_hop = any(re.search(pattern, query_lower)
                            for pattern in self.multi_hop_patterns)
        has_relationship_words = any(
            word in query_lower for word in self.relationship_words)
        has_complex_reasoning = any(re.search(pattern, query_lower)
...
class AgenticQueryRouter:
    ...
    def route_query(self, query: str) -> RouteDecision:
        ...
        logger.info(
            f"Routed query to {route.value} (confidence: {confidence:.2f})")
        return decision
...
class HybridRetriever:
    ...
    async def retrieve(self, query: str, k: int = 10) -> HybridRetrievalResponse:
        ...
        if route_decision.route == RouteType.VECTOR:
            ...
            results = await self._hybrid_retrieve(query, route_decision.features, k)
    ...
    def _reciprocal_rank_fusion(self, vector_results: List[RetrievalResult],
        ...
        sorted_items = sorted(content_scores.items(),
    ...
    def _get_content_key(self, content: str) -> str:
        ...
        # Use first 100 characters as key for approximate matching
...
class PerformanceEvaluator:
    ...
    async def run_evaluation(self, test_queries: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for query_data in test_queries:
            ...
            # Track metrics
            results['retrieval_latencies'].append(
...
async def run_router_demo(data_directory: str = None):
    ...
    try:
        ...
        mode = input(
            "Choose mode: (1) Interactive, (2) Batch all queries, (3) Custom query: ")
        ...
        if mode == "1":
            ...
            while True:
                print("\n" + "-"*50)
                choice = input(
                    f"Enter query number (1-{len(demo_queries)}) or 'q' to quit: ")
            ...
            for i, query_data in enumerate(demo_queries):
                print(
                    f"Query {i+1}/{len(demo_queries)}: {query_data['query']}")
...
async def demo_single_query(hybrid_retriever, query_data: Dict[str, Any], show_details: bool = True):
    ...
    try:
        ...
        if show_details:
            ...
            print(
                f"  Multi-hop Indicators: {features.has_multi_hop_indicators}")
            print(f"  Relationship Words: {features.has_relationship_words}")

            if features.named_entities:
                print(
                    f"  Named Entities: {', '.join(features.named_entities[:5])}")
...
def show_routing_summary(routing_results: List[Dict], demo_queries: List[Dict]):
    ...
    if total_with_expected > 0:
        ...
        print(
            f"🎯 Router Accuracy: {accuracy:.1f}% ({correct_routes}/{total_with_expected})")

    print(f"⚡ Average Processing Time: {total_time/len(routing_results):.3f}s")
    print(
        f"📊 Average Results per Query: {total_results/len(routing_results):.1f}")
    ...
    for result, query_data in zip(routing_results, demo_queries):
        ...
        print(
            f"  {status} Expected: {expected.upper():<6} → Actual: {actual.upper():<6} | {query_data['query'][:50]}...")
