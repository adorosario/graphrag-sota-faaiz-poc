    ...
    try:
        ...
        if st.session_state.hybrid_retriever:
            ...
            try:
                ...
                # Create new VectorRetriever instance which will sync with Neo4j
                st.session_state.hybrid_retriever.vector_retriever = VectorRetriever(config)
                
                st.sidebar.success("✅ Search index updated!")
                st.sidebar.info(f"ChromaDB now has {st.session_state.hybrid_retriever.vector_retriever.collection.count()} documents")
            except Exception as e:
