...
try:
    ...
    st.error("Could not import query_router_system. Please ensure all dependencies are installed.")
    st.stop()
...
def check_authentication():
    ...
    <div class="main-header">
        <h1>🔐 GraphRAG Demo - Authentication Required</h1>
        <p>Please enter your credentials to access the demo</p>
    </div>
    ...
    st.markdown("---")
    st.markdown("**Contact:** For demo access credentials, please contact the administrator.")
    
    return False

# Page configuration
st.set_page_config(
    page_title="GraphRAG Smart Router Demo",
    page_icon="🧠",
    layout="wide",
...
def render_header():
    ...
    st.markdown("""
    <div class="main-header">
        <h1>🧠 GraphRAG Smart Router Demo</h1>
        <p>Intelligent query routing for optimal RAG performance</p>
        <p><strong>Demonstrating measurable improvements from day one</strong></p>
    </div>
    """, unsafe_allow_html=True)
...
def get_demo_queries():
    ...
    return [
        ...
        },
        {
            ...
            'expected_route': 'VECTOR',
            'description': 'Specific financial fact'
        },
        {
        ...
        },
        {
            ...
            'expected_route': 'GRAPH',
            'description': 'Person-to-person connections'
        },
        {
            'query': 'How did IBM technology development affect Fortran programming language adoption?',
            'expected_route': 'HYBRID',
            'description': 'Complex multi-hop reasoning'
        },
        {
            'query': 'Compare budget allocations from 2019 to 2023 and analyze departmental impact',
            'expected_route': 'HYBRID',
            'description': 'Multi-year comparative analysis'
        }
    ]
...
def display_query_results(response, show_details=True):
    ...
    with col1:
        route_badge = render_route_badge(route_decision.route.value, route_decision.confidence)
        ...
        with st.expander("🔧 Query Analysis Details", expanded=False):
            ...
            with col2:
                ...
                st.json({
                    ...
                    "named_entities": features.named_entities[:5]  # Show first 5
                })
    ...
    if response.results:
        ...
        st.warning("🔍 No results found for this query. Try rephrasing or using different keywords.")
...
def render_sidebar():
    ...
    if uploaded_files:
        if st.sidebar.button("🔄 Process Uploaded Documents"):
            with st.spinner("Processing documents..."):  # Use st.spinner instead of st.sidebar.spinner
                success_count = process_uploaded_documents(uploaded_files)
                if success_count > 0:
                    st.sidebar.success(f"✅ Processed {success_count} documents!")
                    st.sidebar.info("System will use new documents in queries.")
                else:
    ...
    **Performance Targets:**
    ...
    if st.session_state.query_history:
        ...
        for route, count in route_counts.items():
            ...
            st.sidebar.progress(percentage/100, text=f"{route.upper()}: {count} ({percentage:.0f}%)")
...
def process_uploaded_documents(uploaded_files):
    ...
    if large_files:
        st.sidebar.error(f"❌ Files too large (>{max_size_mb}MB): {', '.join(large_files)}")
        return 0
...
def main():
    ...
    query_method = st.radio(
        "Choose query method:",
        ["Demo Queries", "Custom Query"],
        horizontal=True
    )
    
    demo_queries = get_demo_queries()
    
    if query_method == "Demo Queries":
        # Show demo queries with descriptions
        st.markdown("#### Available Demo Queries:")
        
        for i, query_data in enumerate(demo_queries):
            col1, col2, col3 = st.columns([6, 2, 2])
            
            with col1:
                st.write(f"**{i+1}.** {query_data['query']}")
                st.caption(query_data['description'])
            
            with col2:
                expected_badge = render_route_badge(query_data['expected_route'], 1.0)
                st.markdown(f"Expected: {expected_badge}", unsafe_allow_html=True)
            
            with col3:
                if st.button(f"Execute", key=f"demo_{i}"):
    ...
    if selected_query and st.button("🚀 Execute Query", type="primary"):
        with st.spinner("Processing query..."):
            try:
                # Execute the query
                response = asyncio.run(execute_query(selected_query, st.session_state.hybrid_retriever))
